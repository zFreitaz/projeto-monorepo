...
export class UserController {
    ...
    public static async show(req: Request, res: Response) : Promise<Response>{
        try {
            ...
            if (isNaN(id) || id <= 0) {
                return res.status(400).json({ errp: 'O ID informado deve ser um número válido'})
            }
    ...
    public static async create(req: Request, res: Response) : Promise<Response>{
        try {
            ...
            if (!senha_hash || typeof senha_hash !== 'string' || senha_hash.length < 6) {
                return res.status(400).json({erro: 'A senha deve conter no minímo 6 caracteres.'});
            }

            const userExistente = await User.findOne({ where: { email: email.trim() }});
            if (userExistente) {
                return res.status(400).json({erro: 'Já esxite um usuário cadastrado nesse e-mail.'});
            }
        ...
        try {
            ...
            if (nome !== undefined) {
                if (typeof nome !== 'string' || nome.trim() === ''){
                    return res.status(404).json({erro: ' O campo nome debe ser um texto valdo.'});
                }
